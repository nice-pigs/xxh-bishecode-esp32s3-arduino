...
/* ============== GPS配置 ============== */
#define GPS_RX_PIN 20       /* GPIO20 - 接GPS模块TX引脚 */
#define GPS_TX_PIN -1       /* 不需要向GPS发送数据 */
double gps_lat = 0;
double gps_lng = 0;
bool gps_valid = false;
bool gps_initialized = false;
TinyGPSPlus gps;
HardwareSerial gpsSerial(1);  /* 使用UART1读取GPS数据 */
#define LOCATION_REPORT_INTERVAL 60000  // 1分钟上报一次
...
#define SENSOR_READ_INTERVAL 60000  /* 改为1分钟，减少输出 */

/* ============== 模式控制变量 ============== */
bool preview_mode = false;
bool need_redraw_main = true;
bool camera_in_jpeg_mode = false;  // 标记摄像头是否在JPEG模式
...
void setup()
{
    ...
    Serial.println("GPS UART初始化完成 (GPIO20, 9600bps)");
    ...
    switch (app_state) {
        ...
        case STATE_IDLE:
            ...
            if (key0_pressed) {
                ...
                // 如果摄像头在JPEG模式，先切换回RGB565
                if (camera_in_jpeg_mode) {
                    Serial.println("[预览] 切换回RGB565格式...");
                    esp_camera_deinit();
                    delay(200);
                    ...
                    if (err != ESP_OK) {
                        Serial.printf("[预览] RGB565初始化失败！错误: 0x%x\n", err);
                        // 失败也继续，可能已经是RGB565了
                    } else {
                        Serial.println("[预览] RGB565初始化成功！");
        ...
        case STATE_PREVIEW:
            ...
            if (!camera_in_jpeg_mode) {
                ...
                // 如果在JPEG模式，显示提示
                lcd_clear(BLACK);
                lcd_show_string(30, 100, 200, 16, LCD_FONT_16, "Camera in JPEG", YELLOW);
                lcd_show_string(30, 120, 200, 16, LCD_FONT_16, "Press KEY0 again", YELLOW);
            }
...
void capture_and_upload()
{
    ...
    // ---------------- 注意：不恢复RGB565，保持JPEG模式 ----------------
    // 避免内存碎片问题，下次进入预览时再切换
    camera_in_jpeg_mode = true;
    Serial.println("[拍照] 保持JPEG模式，避免内存碎片");
